...
class DomainService
{
     ...
     * @param DomainHistoryRepository $histories Репозиторий истории изменения доменов
     */
    public function __construct(
        ...
        private readonly DomainHistoryRepository $histories
    )
    {
    }
     ...
     *
     * @param EntityDTO $params   DTO параметров доменов
     * @param Uuid      $userUuid UUID сотрудника сохраняющего данные
     *
     * @return Domain|null
     * @throws DomainException
     */
    public function save(EntityDTO $params, Uuid $userUuid): ?Domain
    {
        $domain = $params->id ? $this->domains->find($params->id) : new Domain();
        if (null === $domain) {
            throw new DomainException('Отсутствует домен с ID: '. $params->id, DomainException::NOT_EXISTS);
        }
        ...
        if (is_array($validateResult)) {
            throw new DomainException(implode("\n", $validateResult), DomainException::BAD_VALUES);
        }
    ...
    {
        return $this->domains->findByParams($params);
    }
    ...
    {
        return ($result = $this->domains->find($id))
            ? $result->toArray()
            : [];
    }
    ...
    {
        return $this->domains->findFilters();
    }
